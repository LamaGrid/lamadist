...
```mermaid
flowchart LR
    pyproject["container/pyproject.toml"] -- "mise run update-deps" --> reqs["container/requirements.txt"]
    reqs --> container["Container"]
```

- **`container/pyproject.toml`**: High-level Python dependency specifications (PEP 621)
- **`container/requirements.txt`**: Locked, compiled dependencies generated by `mise run update-deps` and committed to the repo.
...
| :--- | :--- | :--- | :--- |
| **APT** (Container) | `container/packages.txt` | `container/packages.lock` | `mise run update-deps` |
| **Python** (Container) | `container/pyproject.toml` | `container/requirements.txt` | `mise run update-deps` |
| **Mise** (Tools) | `.mise.toml` | `mise.lock` | `mise run update-deps` |
