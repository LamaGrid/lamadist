# Backport of the OpenEmbedded-core openssh 10.4p1 upgrade, verbatim
# from OE-core master, onto this image's older LTS base.  10.4 is the
# first release carrying the ssh-mldsa44-ed25519 post-quantum
# signature the SSH authentication policy requires; the pinned LTS
# OE-core still ships 10.3p1.  No PREFERRED_VERSION is set, so this
# recipe wins only by being the higher version and yields
# automatically once the LTS line ships 10.4 or newer, at which point
# this file and its copied openssh/ files should be removed.  Keep it
# in sync with OE-core's openssh recipe until then.
