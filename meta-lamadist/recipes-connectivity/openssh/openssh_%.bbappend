...
# files/10-lamadist-auth.conf.  Cryptographic policy drop-in: no
# algorithm ssh-audit rates as "fail", Edwards-curve host key only;
# see files/20-lamadist-crypto.conf.  The stock sshd_config carries
# "Include /etc/ssh/sshd_config.d/*.conf" as its first directive and
# sshd_config_readonly is a copy of it, so the drop-ins bind first
...
do_install:append() {
	...
	install -m 0644 ${UNPACKDIR}/20-lamadist-crypto.conf \
		${D}${sysconfdir}/ssh/sshd_config.d/
}
